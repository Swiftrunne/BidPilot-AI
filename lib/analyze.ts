...
export async function analyzeSolicitationText(text: string): Promise<BidAnalysis> {
  ...
  const completion = await client.chat.completions.create({ model, temperature: 0.2, messages: [{ role: 'system', content: 'Extract solicitation requirements, risks, line items, compliance tasks, and pricing-relevant data. Return strict JSON only.' }, { role: 'user', content: prompt }] });
